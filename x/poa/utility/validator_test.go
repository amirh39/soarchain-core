...
func Test_IsNotValidPubkey(t *testing.T) {
	r := require.New(t)
...
func Test_IsValidAddress(t *testing.T) {
	...
	result := IsValidAddress(address)
	r.Equal(result, true)
	r.Equal(len(address), 43)
	r.Equal(address[0:4], "soar")
}

func Test_IsNotValidAddress(t *testing.T) {
	r := require.New(t)

	emptyAddress := ""
	r.Equal(IsValidAddress(emptyAddress), false)

	shortAddress := "soar1uajy2t7tyamnuqms7l65ka4wtwgrvey0rve34"
	r.Equal(IsValidAddress(shortAddress), false)

	firstNotEqualAddress := "doar1uajy2t7tyamnuqms7l65ka4wtwgrvey0rve34t"
	r.Equal(IsValidAddress(firstNotEqualAddress), false)
}
