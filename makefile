...
keys:
	...
	soarchaind keys add v2xDevice1 --keyring-backend test --algo secp256k1
	soarchaind keys add v2nDevice1 --keyring-backend test --algo secp256k1
	soarchaind keys add runnerDevice1 --keyring-backend test --algo secp256k1
...
sign_genesis_transaction:
	soarchaind gentx zeus 10000000umotus --keyring-backend test --chain-id soarchaindevnet
...
vote_proposal:	
	soarchaind tx gov deposit 1 10000000umotus --from zeus --chain-id soarchaindevnet --yes
